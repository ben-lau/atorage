...
export function withScope<T = unknown>(...scopes: Scope[]): AtomModifier<T> {
  return (config) => ({ ...config, scopes: [...config.scopes, ...scopes] });
}

export function withMiddleware<T = unknown>(...mw: Middleware[]): AtomModifier<T> {
  return (config) => ({ ...config, middleware: [...config.middleware, ...mw] });
}

export function withPreMiddleware<T = unknown>(...mw: Middleware[]): AtomModifier<T> {
  return (config) => ({
    ...config,
