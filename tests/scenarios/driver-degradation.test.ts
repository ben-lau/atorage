...
function createFlakyDriver(name: string, failPattern: { get?: boolean; set?: boolean; del?: boolean }): Driver & { store: Map<string, unknown> } {
...
function createLatencyDriver(name: string, latencyMs: number): Driver & { store: Map<string, unknown> } {
  const store = new Map<string, unknown>()
  const delay = () => new Promise(r => setTimeout(r, latencyMs))
  return {
    name,
    store,
    async get(key) { await delay(); return store.get(key) },
    async set(key, value) { await delay(); store.set(key, value) },
    async del(key) { await delay(); store.delete(key) },
    async has(key) { await delay(); return store.has(key) },
    async keys() { await delay(); return [...store.keys()] },
    async dispose() { store.clear() },
  }
}
...
describe('Scenario: driver degradation under real conditions', () => {
  ...
  describe('Dynamic driver available() detection', () => {
    ...
    it('driver with available() throwing an exception is treated as unavailable', async () => {
      const explosive: Driver = {
        name: 'explosive',
        available: () => { throw new Error('quota exceeded') },
  ...
  describe('Edge cases', () => {
    it('behavior of all operations when there are zero drivers', async () => {
      ...
      const a = atom<string>('key', withDriver({
        name: 'ghost',
        available: () => false,
        async get() { return undefined },
        async set() {},
  ...
  describe('Multi-driver read priority', () => {
    it('first driver with value does not read subsequent drivers', async () => {
      ...
      const secondary: Driver = {
        ...
        async get() { secondRead = true; return { $v: 'secondary' } },
        async set() {},
        async del() {},
        async has() { return true },
        async keys() { return [] },
