...
function intermittentDriver(): Driver & { failNext: () => void } {
  ...
  return {
    ...
    async get(key) {
      if (shouldFail) { shouldFail = false; throw new Error('intermittent read failure') }
      return store.get(key)
    },
    async set(key, value) {
      if (shouldFail) { shouldFail = false; throw new Error('intermittent write failure') }
      store.set(key, value)
    },
    async del(key) {
      if (shouldFail) { shouldFail = false; throw new Error('intermittent del failure') }
      store.delete(key)
    },
  ...
  describe('Lockless get/set races', () => {
    it('concurrent set without protection: final value depends on which driver.set runs last', async () => {
      ...
      const results = await Promise.allSettled(
        Array.from({ length: 10 }, (_, i) => a.set(i))
      )
